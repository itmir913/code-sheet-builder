...
그래서 **실행 시점에 CDN을 부르는 코드를 넣으면 안 됩니다.** 폰트도 라이브러리도 전부 npm으로 받아 번들에 넣습니다. `file://` 로 열리는 환경을 위해 빌드 설정에 몇 가지 제약이 걸려 있는데, 이유는 `vite.config.js` 주석에 적어 두었습니다.
...
tests/              Vitest 테스트
public/             그대로 dist로 복사되는 파일 (예제코드 zip)
```
